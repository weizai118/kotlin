...
class WrapWithCollectionLiteralCallFix private constructor(
    ...
    companion object {
        fun create(expectedType: KotlinType, expressionType: KotlinType, element: KtExpression): WrapWithCollectionLiteralCallFix? {
            val collectionType =
            ...
            if (expressionType.isSubtypeOf(expectedArgumentType) && collectionType.literalFunctionName != null) {
                return WrapWithCollectionLiteralCallFix(element, collectionType.literalFunctionName, wrapInitialElement = true)
            }

            // Replace "null" with emptyList()
            if (element.isNullExpression() && collectionType.emptyCollectionFunction != null) {
                return WrapWithCollectionLiteralCallFix(element, collectionType.emptyCollectionFunction, wrapInitialElement = false)
            }
