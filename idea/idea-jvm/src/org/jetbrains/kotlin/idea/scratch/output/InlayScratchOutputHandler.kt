...
object InlayScratchOutputHandler : ScratchOutputHandler {
    ...
    private fun createInlay(file: ScratchFile, expression: ScratchExpression, output: ScratchOutput) {
        UIUtil.invokeLaterIfNeeded {
            ...
            fun addInlay(text: String) {
                val shortText = StringUtil.shortenTextWithEllipsis(text.substringBefore("\n"), maxLineLength - spaceCount - lineLength, 0)
                if (shortText != text) {
    ...
    private fun maxLineLength(file: ScratchFile): Int {
        ...
        return file.getExpressions().flatMap {
            it.lineStart..it.lineEnd
        }.map {
            doc.getLineEndOffset(it) - doc.getLineStartOffset(it)
        }.max() ?: 0
    }
